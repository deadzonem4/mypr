jQuery.fn.dataManagerItem = function(options) {
    ...
    $el.on('click', '.js-item-remove', function a() {
        ...
        $('.es-confirm-popup').esPopup('yesCallback', function() {
            ...
            if (id != undefined) {
                $.post(ajaxurl, {id: id, action: action, container: container, storage: storage}, function(response) {
                    response = JSON.parse(response);
    ...
    $el.on('click', '.js-item-radio', function() {
        ...
        $.post(ajaxurl, {checked: $(this).is(':checked'), id: $(this).val(), action: action, container: container, storage: storage}, function(response) {
            response = JSON.parse(response);
