...
class Es_Property_List_Page extends Es_Object
{
    ...
    public function post_row_actions( $actions, $post ) {
        if ( $post->post_type == $this->get_post_type() ) {
            ...
            $actions['trash'] = '<a href="' . get_delete_post_link( $post->ID ) . '">
    ...
    {
        if ( 'thumbnail' == $column )
            es_the_post_thumbnail( static::table_image_size() );
    ...
    public function manage_form_submit()
    {
        ...
        if ( ! empty( $_GET['post_type'] ) && ! empty( $_GET['es-action'] ) && ! empty( $_GET['post'] ) ) {
            ...
            switch( $_GET['es-action'] ) {
                // Copy selected posts.
                case 'copy':
                    array_walk( $_GET['post'], function( $post_id ) {
