<div class="es-property-fields col-sm-5">
    ...
    ?>
        <div class="single-label-sold"><?php _e('Sold','Sold'); ?></div>

    <?php elseif(get_field('property_sold') == 'rented'): ?>
        <div class="single-label-sold"><?php _e('Rented out','Rented out'); ?></div>
    <?php endif; ?>
         ...
         <?php if ( $fields = Es_Property_Single_Page::get_single_fields_data() ) : ?>
            <?php foreach ( $fields as $field ) : ?>
                <?php if ( ! empty( $field[ key( $field ) ] ) ) : ?>
                    <li><strong><?php echo key( $field ); ?>: </strong><span class="field-value"><?php echo $field[ key( $field ) ]; ?></span></li>
                <?php endif; ?>
            <?php endforeach; ?>
        <?php endif; ?>
    </ul>
